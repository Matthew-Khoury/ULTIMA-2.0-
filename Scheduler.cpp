...
int create_task(){
    if(next_available_task_id<MAX_TASKS){
        ...
        task_table[next_available_task_id].task.id = next_available_task_id << endl;
...
void yield (){
    ...
    clock_t elaspsed_time = clock() - task_table[current_task].start_time;
    cout << "Task: " << current_task << ", Elapsed time: "<< elaspsed_time << endl;
    ...
    if(elaspsed_time >= current_quantum){
        ...
        while(task_table[current_task].state != READY && counter < MAX_TASKS-1){ //we are subtracting one because we can not surpase the max tasks
            current_task = (current_task + 1) % MAX_TASKS;
            counter ++;
    ...
    cout<<"NO YIELD!  (task: " << current_task << " Still have some quantum left)" << endl;
}
